...
import { ArrowLeft, Mail, Phone, Building2, User, GraduationCap, BadgeCheck, Clock, MessagesSquare, CalendarClock } from "lucide-react";
import api from "../../api";
...
export default function StudentProfile() {
  ...
  useEffect(() => {
    ...
    api.get("/attendance").then((res) => {
      const records = res.data.filter((r) => r.student_id === Number(id) || r.intern_id === Number(id));
      setAttendance(records.sort((a, b) => new Date(b.date || b.created_at) - new Date(a.date || a.created_at)));
    }).catch(() => setAttendance([]));
  }, [id]);

  if (student === null) return <div className="grid min-h-[60vh] place-items-center"><Spinner /></div>;
  if (!student) return (
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        <button onClick={() => navigate(-1)} className="flex items-center gap-1 text-sm font-medium text-slate-500 hover:text-slate-700">
        ...
        <div className="px-6 pb-6">
          <div className="-mt-14 mb-4 flex flex-col items-start gap-4 sm:flex-row sm:items-end">
            ...
            <div className="flex items-center gap-2">
              <Link to={`/messages?to=${student.id}`} className="btn-ghost flex items-center gap-1.5"><MessagesSquare size={16} /> Message</Link>
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {[
              { icon: BadgeCheck, label: "Student ID", value: student.student_id || student.id },
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        <div className="card p-5">
          ...
          {attendance === null ? (
            ...
            <p className="text-sm text-slate-500">No attendance records for this student.</p>
          ) : (
