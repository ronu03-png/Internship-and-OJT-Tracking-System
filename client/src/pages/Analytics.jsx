...
function SimpleBar({ label, value, max, color = "bg-brand-600" }) {
...
export default function Analytics() {
  ...
  const maxHours = Math.max(...students.map((s) => s.required_hours || 1), 1);
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-6 lg:grid-cols-2">
          ...
          {students.length === 0 ? (
            ...
            <div className="space-y-3">
              {students.slice(0, 10).map((s) => (
                <SimpleBar key={s.id} label={s.full_name} value={`${s.approved_hours}/${s.required_hours} hrs`} max={s.required_hours} />
              ))}
            </div>
          )}
        </div>
        ...
        <div className="card p-5">
          ...
          <SimpleBar label="Openings / Placements" value={stats.placement_count} max={Math.max(stats.placement_count, 1)} color="bg-emerald-500" />
          <SimpleBar label="Announcements" value={stats.announcement_count} max={Math.max(stats.announcement_count, 1)} color="bg-amber-500" />
          <SimpleBar label="Calendar Events" value={stats.event_count} max={Math.max(stats.event_count, 1)} color="bg-sky-500" />
          <SimpleBar label="Audit Logs" value={stats.audit_count} max={Math.max(stats.audit_count, 1)} color="bg-purple-500" />
        </div>
      </div>
    </div>
  );
